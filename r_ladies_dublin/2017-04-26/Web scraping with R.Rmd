...
data <- html_data %>% html_nodes("table.match-table") %>% (function(mach_table) {
  # Use html_text(TRUE) to get rid of leading and trailing whitespace
  date <- mach_table %>% html_nodes("th.comp-date") %>% html_text(TRUE)
  
  maches <- mach_table %>% html_nodes("tr.clickable") %>% (function(mach) {
    list(home_team = mach %>% html_nodes("td.team > div.home > span") %>% html_text(TRUE),
         away_team = mach %>% html_nodes("td.team > div.away > span") %>% html_text(TRUE),
         score     = mach %>% html_nodes("td.vs > div") %>% html_text(TRUE)
    )
  })
  
  list(date = date, maches = maches)
})
